...
private final long firingDurationMs = 100;
    ...
    private final long collisionCooldown = 3000;
    private boolean isBlinking = false;
    private long lastHitTime;
    private final long blinkInterval = 3000;
    ...
    public void update() {
        ...
        if (keyHandler.fire && !lastFireState  && !keyHandler.left && !keyHandler.right ) {
        IsFiring = true;
    ...
    public void checkCollisionWithBullets(List<EnemyBullet> enemies){
        long currentTime = System.currentTimeMillis();

        if (currentTime - lastCollisionDetectionTime >= collisionCooldown) {
            for(EnemyBullet enemy:enemies){
            if (enemy.colition != null && this.colition.intersects(enemy.colition)) {
